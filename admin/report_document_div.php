<?php
$_SESSION['SES_TITLE'] = "Statistic of document by division/department ";
...
$title  = isset($_GET['t']) ?  $_GET['t'] : 'Report of document by division / department';
$_SESSION['SES_SUBTITLE'] = "Division : " . $id . " & date : " . $tgl1 . " - " . $tgl2;
$_SESSION['SES_PAGE'] = "?page=Report-Document-Div&fr=" . $tgl1 . "&to=" . $tgl2 . "&id=" . $id . "&id2=" . $id2;
?>
  ...
  <div class="">
    <div class="page-title">
      <div class="title_left">
        <h3>Statistic of Documents<small> ( By Division / Department )</small></h3>
      </div>
            ...
            <form role="form" action="?page=Filter" method="POST" name="form1" target="_self" id="form1">
              ...
              <div class="col-sm-4">
                <div class="form-group">
                  ...
                  $mySql = "SELECT division FROM view_log_document WHERE division<>'' group by division order by division";
              ...
              <div class="col-sm-4">
                <div class="form-group">
                  <label>Unit</label>
                  <?php // get the users
                  $data    = isset($_GET['id2']) ?  $_GET['id2'] : '%';
                  $select_box = "<select name=\"txtID2\" id=\"txtID2\"  class=\"select2_single form-control \" >
								<option value=\"%\">All Unit</option>";
                  $mySql = "SELECT department FROM view_log_document WHERE department<>''group by department order by department";
                  $dataQry = mysqli_query($koneksidb, $mySql) or die("Error Query" . mysqli_error($koneksidb));
                  $result = mysqli_query($koneksidb, $mySql) or die('Query failed: Could not get list of data : ' . mysqli_error($con)); // query
                  while ($row = mysqli_fetch_array($result)) {
                    foreach ($row as $key => $value) {
              ...
              <div class="col-sm-2">
                <div class="form-group">
                  <label>&nbsp;</label><br />
                  <button type="submit" class="btn btn-primary btn-sm" name="btnDocumentDiv" style="width:100%"><i class="fa fa-filter fa-fw"></i> Filter</button>
                </div>
              </div>
              <div class="col-sm-2">
                <div class="form-group">
                  <label>&nbsp;</label><br />
                  <button type="submit" class="btn btn-info btn-sm" name="btnDocumentDivSubmit" style="width:100%"><i class="fa fa-file-pdf-o fa-fw"></i> PDF</button>
          ...
          <div class="x_content table-responsive">
            <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
              ...
              <tbody>
                ...
                $id    = isset($_GET['id']) ?  $_GET['id'] : '%';
                $id2  = isset($_GET['id2']) ?  $_GET['id2'] : '%';
                $mySql   = "SELECT * FROM view_log_document_perdivision WHERE tgl between '$tgl1' and '$tgl2' and division like '$id' 
									and department like '$id2' ORDER BY tgl, document_id, document_version ";
